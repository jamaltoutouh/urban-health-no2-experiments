...
df_pre_mc = dataset[(dataset.index >= "2013-12-01") & (dataset.index <= "2018-11-30") & (dataset.season == season)]
df_post_mc = dataset[(dataset.index >= "2018-12-01") & (dataset.index <= "2019-11-30") & (dataset.season == season)]
...
def get_stats(data_list, threshold):
    ...
    test = sum(val >= threshold)/len(val)

    return minn, mean, norm_stdev, maxx, test
...
for i, day in enumerate(day_type):
    ...
    if gap > 0 or pre_test < post_test:
        stats = '{} & {} & {} & {:.2f} & {} & {:.2f} & {:.2f} \\\ '.format(station, season, day, gap, res,
                                                                              pre_test, post_test)
        print(stats)
    ...
    # rowcolor = 'rowcolor{gray!50} \n ' if gap > 0 else ''
    # stats = '{} {} & {} & {} & {:.2f} & {} & {:.2f} & {:.2f} \\\ '.format(rowcolor, station, season, day, gap, res, pre_test, post_test)
    # print(stats)
